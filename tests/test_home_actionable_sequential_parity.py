...
class TestHomeActionableSequentialParity:

    @pytest.mark.asyncio
    @pytest.mark.parametrize("user_id", TEST_USER_IDS)
    async def test_mismas_tres_cajas(self, db_ready, user_id):
        from services.home.service import get_home_actionable

        sequential, parallel = await leer_estable(
            lambda paralelo: get_home_actionable(
                user_id, limit=5, schema_name=SCHEMA, _force_parallel_fetch=paralelo),
            etiqueta=f"actionable user={user_id}",
        )
