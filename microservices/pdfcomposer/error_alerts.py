...
logger = logging.getLogger("error_alerts")

RESEND_API_URL = "https://api.resend.com/emails"

_CERT_PATH_RE = re.compile(r"(?:/dev/shm|/tmp)/\S*\.p12", re.IGNORECASE)
...
def _sanitize_error(msg: str) -> str:
    if not msg:
        return msg
    msg = _PEM_BLOCK_RE.sub("<pem_redacted>", msg)
    msg = _CERT_PATH_RE.sub("<cert_path_redacted>", msg)
    msg = _HEX_RE.sub("<hex_redacted>", msg)
    return msg
...
def report_error(request, exc: Exception, *, kind: str = "UNHANDLED") -> None:
    ...
    try:
        ...
        tb = _sanitize_error("".join(traceback.format_exception(type(exc), exc, exc.__traceback__)))[-3500:]
...
def _build_html(cfg, kind, exc, method, path, schema, user_id, occurrences, tb) -> str:
    ...
    def esc_msg(v):
        return html.escape(_sanitize_error(str(v))) if v is not None else "—"
    ...
    <div style="font-family:system-ui,Segoe UI,Arial,sans-serif;font-size:14px;color:#111;max-width:720px">
      ...
      <table cellpadding="6" style="border-collapse:collapse;width:100%">
        ...
        <tr><td style="background:#f3f4f6"><b>Mensaje</b></td><td>{esc_msg(exc)}</td></tr>
