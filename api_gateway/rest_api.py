...
async def api_get_case_history(request: Request) -> JSONResponse:
    ...
    try:
        result = await cases.get_case_history(ctx=ctx, case_id=case_id, user_id=user_id)
        return _success_response(result)
...
async def api_search_documents(request: Request) -> JSONResponse:
    ...
    Query params:
        ...
        - status: str ("En edición", "Firmar ahora", "En proceso de firma", "Firmado")
...
async def api_get_case_responsibles(request: Request) -> JSONResponse:
    """
    ...
    try:
        result = await cases.get_case_responsibles_list(ctx=ctx, case_id=case_id, user_id=user_id)
        return _success_response(result)
    except AuthorizationError:
        return _error_response("Sin permisos para ver este expediente", status_code=403)
    except Exception:
        logger.exception("[REST API] Error en get_case_responsibles")
        return _error_response("Error interno del servidor", status_code=500)


async def api_add_case_responsible(request: Request) -> JSONResponse:
    """
    ...
    Body JSON:
        - user_id: UUID del usuario a agregar
        - type: ADMIN o ADDITIONAL
        - sector_id: UUID del sector del usuario
        - reason: Motivo (opcional, default "Asignación de responsable")
...
async def api_remove_case_responsible(request: Request) -> JSONResponse:
    """
    ...
    Query params:
        - reason: Motivo de remoción (opcional)
...
async def api_subsanar_document(request: Request) -> JSONResponse:
    """
    POST /api/v1/cases/{case_id}/subsanar

    Subsana un documento oficial erróneo en un expediente.
    Desactiva el documento erróneo y vincula el documento que justifica
    la subsanación. Solo usuarios ADMIN del expediente pueden ejecutarlo.
    ...
    try:
        ...
        logger.exception("[REST API] Error en subsanar_document")
        return _error_response("Error interno del servidor", status_code=500)
...
async def api_get_case_movements(request: Request) -> JSONResponse:
    """
    GET /api/v1/cases/{case_id}/movements

    Lista plana de movimientos de un expediente (sin ai_summary).
    Distinto de /history que incluye análisis IA.
    ...
    try:
        ...
        # X-User-ID válido en api_key_users pero inexistente/inactivo en el schema
        return _error_response(str(e), status_code=401)
    except NotFoundError as e:
        return _error_response(str(e), status_code=404)
    except AuthorizationError:
...
async def api_sign_document(request: Request) -> JSONResponse:
    ...
    # Verificar signature_policy: la REST API solo permite firma electronica.
    # Documentos que requieren token fisico (digital_all, o digital_num+numerador)
    # deben firmarse desde el frontend con AutoFirma.
    try:
        from database import fetch_one as _fetch_one_sign
        row = await _fetch_one_sign(
                    """
        ...
        if not row:
            return _error_response("Documento no encontrado o usuario no es firmante", status_code=403)
        policy = row["signature_policy"] or "electronic"
        ...
        logger.exception(f"[REST API] sign_document policy_check error")
...
async def api_link_document(request: Request) -> JSONResponse:
    ...
    try:
        result = await cases.link_document_to_case(
            ctx=ctx,
            case_id=case_id,
...
async def api_prepare_transfer(request: Request) -> JSONResponse:
    ...
    try:
        result = await cases.prepare_transfer(
            ctx=ctx,
            case_id=case_id,
            user_id=user_id
        )
...
async def api_accept_proposal(request: Request) -> JSONResponse:
    ...
    try:
        result = await cases.accept_proposal(
            ctx=ctx,
            case_id=case_id,
...
async def api_get_sectors(request: Request) -> JSONResponse:
    ...
    try:
        result = await system.get_sectors(ctx=ctx)
        return _success_response(result)
...
async def api_get_dashboard_stats(request: Request) -> JSONResponse:
    ...
    try:
        result = await system.get_dashboard_stats(ctx=ctx, user_id=user_id)
        return _success_response(result)
...
async def api_get_dashboard_feed(request: Request) -> JSONResponse:
    ...
    try:
        result = await system.get_dashboard_feed(
            ctx=ctx,
            user_id=user_id,
            page=page,
            page_size=page_size
...
async def api_check_signer_permissions(request: Request) -> JSONResponse:
    ...
    try:
        result = await system.check_signer_permissions(
            ctx=ctx,
...
async def api_list_all_users(request: Request) -> JSONResponse:
    ...
    try:
        result = await system.list_all_users(ctx=ctx)
        return _success_response(result)
...
async def api_sync_documents(request: Request) -> JSONResponse:
    """GET /api/v1/sync/documents — PDFs firmados con presigned URLs (Backup)."""
    try:
        backup_ctx = await validate_backup_api_key(request)
    except BackupAuthError as e:
        return _backup_error_response(e)
...
async def api_semantic_search(request: Request) -> JSONResponse:
    ...
    Query params:
        ...
        - limit: int (default 20, max 50) - max resultados
...
async def api_get_record_documents(request: Request) -> JSONResponse:
    ...
    try:
        ...
        result = await get_linked_documents(
            record_id,
            ctx.user_id,
            schema_name=ctx.schema_name,
            page=int(params.get("page", 1)),
